...
export class CreateCalculoComponent implements AfterViewInit {
  ...
  guardarCalculo() {
    ...
    if (this.calculo.tipo == "potencias") {
      this.calculo.potencias = this.valores
    }
    else if (this.calculo.tipo == 'facturas') {
      let valor_mes = [
      ...
      this.calculo.potencias = this.valores_mensuales
    }
    //Determinamos si hay potencias agregadas
    if (this.calculo.potencias.length >= 1) {
      this.camposRequeridos.potencias = true
    } else {
    ...
    if (pendientes.size >= 1) {
      ...
      this.calculo.simultaneo = this.simultaneo
      this.calculo.total_dia = this.total_dia
